...
int write_packet( pcap_t *f_out, struct pcap_pkthdr *pkh, unsigned char *h80211 )
{
  ...
  int res = pcap_sendpacket(f_out, (const u_char*)buffer, pkh->caplen);
  if (res == -1) {
    printf("pcap_sendpacket return -1\n");
    return( 1 );
  }
...
usage:
  ...
  char output[65536];
  if ( opt.decrypted_fpath[0])
    strncpy(output, opt.decrypted_fpath, 65536);
  else
    strncpy(output, (char*)buffer, 65536);

  f_out = pcap_open_live(output, 65535, 1, 1, errbuf);
  if( f_out == NULL )
  {
    perror( "pcap_open failed" );
  ...
  while( 1 )
  {
    ...
    struct pcap_pkthdr *_pkh;
    const u_char *buffer;
    int res = pcap_next_ex(f_in, &_pkh, &buffer);
    bool end = false;
    switch (res) {
      case 0: continue;
      case -1:
      ...
      if( ( h80211[z + 3] & 0x20 ) == 0 )
      {
                         ...
                         K, 3 + opt.weplen ) == 0 )
        {
          if(opt.store_bad)
          {
            ...
            //if( write_packet( f_bad, &pkh, h80211 ) != 0 ) // gilgil temp
            //  break;
          }
          continue;
        }
